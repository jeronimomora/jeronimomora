...
class BoxWithDefaultColor extends Mesh {
  defaultColor: string;
  constructor(sideSize: number, defaultColor: string) {
...
const colorBox = (box: BoxWithDefaultColor, color?: string) => {
  (box.material as MeshPhongMaterial).color = new Color(
    ...
    const render = () => {
      if (doRaycast.current) {
        ...
        if (intersectObjects.length) {
          const intersectedBox = intersectObjects[0]
            .object as BoxWithDefaultColor;
